% --------------------------------------------------------------------
function [im, labels] = getBatchInternal(opts, imdb, batch)
...
%    factor_c = [opts.imageSize(1)/crop_height, opts.imageSize(2)/crop_width];
  ...
  if (rand() < opts.augmentation.rotation)
    deg = min(max(normrnd(0, 10), -20), 20);
    imc = rotateImages({imt}, deg);
    imt = imc{1};
  ...
  if ((wt ~= opts.imageSize(2)) || (ht ~= opts.imageSize(1)))
    factor = [(opts.imageSize(1)-1)/ht, (opts.imageSize(2)-1)/wt];
    imt = imresize(imt, 'scale', factor);
  end
  wt = size(imt, 2);
  ht = size(imt, 1);
  if ((wt ~= opts.imageSize(2)) || (ht ~= opts.imageSize(1)))
    factor = [opts.imageSize(1)/ht, opts.imageSize(2)/wt];
    imt = imresize(imt, 'scale', factor);
  end
